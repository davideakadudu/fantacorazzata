...
export function Switch({ checked=false, onCheckedChange, className="", ...rest}: Props){
  return (
    <button
      ...
      className={`inline-flex h-5 w-9 items-center rounded-full border transition ${checked? "bg-emerald-500 border-emerald-600":"bg-slate-200 border-slate-300"} ${className}`}
      {...rest}
    >
