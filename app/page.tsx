...
/* ------------------------------------------------------
   Fantacazzo – Asta Assistant v0.6.7 (fix build)
   - Fix sintassi (updateWatch / addWatchItemToRoster)
   - Prezzo editabile in shortlist
   - Bottone “Mio” aggiunge alla Rosa e rimuove da Shortlist/Watchlist
   - Excel multi-foglio → Watchlist (Portieri/Difensori/Centrocampisti/Attaccanti)
------------------------------------------------------ */
...
export default function Page() {
  ...
  // 🔧 FIX: aggiorna watchlist e shortlist insieme
  ...
  // 🔧 FIX: aggiungi alla rosa e rimuovi da liste
  function addWatchItemToRoster(w: WatchItem) {
    ...
    setPlayers(prev => {
      const next = [...prev, { id: uid(), name: w.name, role: w.role, price, status: "acquisto" }];
    ...
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white dark:from-black dark:to-black">
      ...
      <main className="max-w-7xl mx-auto px-4 pb-10 space-y-6">
        ...
        <footer className="text-xs text-muted-foreground text-center">
          v0.6.7 • Fix compilazione • Shortlist editabile • “Mio” rimuove da liste
        </footer>
      </main>
    </div>
  );
...
function RosterSection({ players, setPlayers, minBid }: { players: Player[]; setPlayers: React.Dispatch<React.SetStateAction<Player[]>>; minBid: number; }) {
  ...
  function addPurchase() {
    ...
    setPlayers(prev => {
      const next = [...prev, { id: uid(), name: newP.name, role: newP.role, price, status: newP.mine ? "acquisto" : "confermato" }];
      next.sort((a,b)=> ROLE_INDEX[a.role]-ROLE_INDEX[b.role] || a.name.localeCompare(b.name));
      return next;
    });
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="space-y-2">
            <div className="border rounded-2xl overflow-hidden">
              {/* Nessun max-h: si vede tutta la lista */}
              <table className="w-full text-sm">
                <thead className="bg-muted sticky top-0 z-10">
                  <tr>
                    <th className="text-left p-2">Giocatore</th>
